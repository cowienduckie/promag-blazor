...
public class MainTasksController : BaseController
{
    ...
    public async Task<ActionResult> UpdateAsync(int id, [FromBody] MainTaskUpdateDto? updateDto)
    {
        try
        {
            ...
            return await Supervisor.UpdateAsync<MainTask, MainTaskUpdateDto>(id, updateDto) ? NoContent() : StatusCode(500);
        }
        catch (Exception e)
        {
            return HandleException(e);
    ...
    [HttpPatch("{id}")]
    public async Task<ActionResult> PartialUpdateAsync(int id, [FromBody] JsonPatchDocument<MainTaskUpdateDto>? updatePatchDoc)
    {
        try
        {
